...
function postComment(url, request) {
  ...
  comment.id = database.nextCommentId
  comment.upvotedBy = []
  comment.downvotedBy = []
...
function putComment(url, request) {
  const urlId = Number(url.split('/').filter(segment => segment)[1]);
  ...
  database.comments[urlId].body = comment.body
  return {
    status: 200,
    body: {}
  }
...
function deleteComment(url, request) {
  /*
  1) routes['/comments/:id'] should have a method called DELETE
  2) routes['/comments/:id'].DELETE should delete an existing comment
  3) routes['/comments/:id'].DELETE should remove a deleted comment ID from the author's comment IDs
  4) routes['/comments/:id'].DELETE should remove a deleted comment ID from the article's comment IDs
  5) routes['/comments/:id'].DELETE should return a 204 response after a succesful delete
  6) routes['/comments/:id'].DELETE should return a 404 response with a non-existent comment ID
  */
  const urlId = Number(url.split('/').filter(segment => segment)[1]);
  ...
  comment = database.comments[urlId]
  database.comments[urlId] = null
  let index = database.users[comment.username].commentIds.indexOf(urlId)
  database.users[comment.username].commentIds.splice(index, 1)
  index = database.articles[comment.articleId].commentIds.indexOf(urlId)
  database.articles[comment.articleId].commentIds.splice(index, 1)
  return {
    status: 204,
    body: {}
  }
...
function upvoteComment(url, request) {
  ...
  if (!request.body ||
      ...
      !database.comments[urlId] ||
      !database.users[request.body.username] ||
      database.comments[urlId].upvotedBy.includes(request.body.username)) {
    return {
      status: 400,
      body: {}
    }
  ...
  database.comments[urlId].upvotedBy.push(username)
  if (database.comments[urlId].downvotedBy.includes(username)) {
    let i = database.comments[urlId].downvotedBy.indexOf(username)
    database.comments[urlId].downvotedBy.splice(i, 1)
  }
...
function downvoteComment(url, request) {
  ...
  if (!request.body ||
      ...
      !database.comments[urlId] ||
      !database.users[request.body.username] ||
      database.comments[urlId].downvotedBy.includes(request.body.username)) {
    return {
      status: 400,
      body: {}
    }
  ...
  let username = request.body.username
  database.comments[urlId].downvotedBy.push(username)
  if (database.comments[urlId].upvotedBy.includes(username)) {
    let i = database.comments[urlId].upvotedBy.indexOf(username)
    database.comments[urlId].upvotedBy.splice(i, 1)
  }
